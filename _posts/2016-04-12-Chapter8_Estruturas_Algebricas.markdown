...
1. $e$ é elemento neutro da operação $\oplus$.


### Exercício:
1. $(\mathbb{N},+)$ não tem elemento neutro.
1. $(\mathbb{Z},+,0)$ é um monóide porque $(\mathbb{Z},+)$ é um semi-grupo e a adição tem por elemento neutro 0.
1. $(\mathbb{Z},\times,1)$ é um monóide porque $(\mathbb{Z},\times)$ é um semi-grupo e a multiplicação tem por elemento neutro 1.
...
1. Se $r_1$ e $r_2$ são expressões regulares associadas a linguagens $L_1$ e $L_2$, respectivamente, então também são expressões regulares,
    ...
    a. $(r_1\cdot r_2)$ associada à linguagem $L_1\cdot L_2$,
    a. $$(r_1^\ast)$ associada à linguagem $L_1^*$$, e
    a. $(r_1^+)$ associada à linguagem $L_1^+$.
...
1. $S$ é um símbolo não terminal, elemento de $V$, denominado símbolo inicial.
